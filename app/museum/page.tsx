...
const generateCameraWaypoints = (artworks: ReturnType<typeof generateArtworkPositions>) => {
  return artworks.map((artwork) => {
    const zOffset = 2.5; // kamera lebih dekat ke artwork
...
function LightingSetup() {
  return (
    <>
      <ambientLight intensity={0.68} color="#fefefe" />
      <directionalLight
        position={[30, 35, 30]}
        intensity={1.15}
        color="#fffaf2"
...
export default function MuseumPage() {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-gradient-to-b from-zinc-900 to-black">
      <Canvas
        camera={{ position: [0, 2.8, 12], fov: 65, far: 600 }}
        gl={{ antialias: true, alpha: false, toneMapping: THREE.ACESFilmicToneMapping, toneMappingExposure: 1.28 }}
        shadows
      >
        {hasEntered && (
          <GalleryScene
